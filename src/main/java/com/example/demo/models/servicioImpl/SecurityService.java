...
public class SecurityService {
    ...
    /**
     * Obtener IPs actualmente bloqueadas (para interfaz admin)
     */
    public List<Map<String, Object>> obtenerIpsBloqueadas() {
        List<Map<String, Object>> bloqueadas = new ArrayList<>();
        long now = System.currentTimeMillis();

        for (Map.Entry<String, AttemptInfo> entry : loginAttempts.entrySet()) {
            String ip = entry.getKey();
            AttemptInfo info = entry.getValue();

            if (info.isBlocked()) {
                long restante = LOCKOUT_DURATION - (now - info.getLastAttemptTime());
                long minutos = Math.max(0, restante / (60 * 1000));

                Map<String, Object> ipInfo = new HashMap<>();
                ipInfo.put("ip", ip);
                ipInfo.put("minutosRestantes", minutos);
                bloqueadas.add(ipInfo);
            }
        }
